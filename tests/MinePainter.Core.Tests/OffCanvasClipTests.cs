...
public class OffCanvasClipTests
{
    ...
    /// <summary>
    /// 大半看得到的物件不裁 —— 裁了的話拖曳快照拿不到快取，得整份現算，
    /// 使用者看到的就是「拖一下閃一下、先出現沒有外框的字」（2026-09-04 回報過的迴歸）。
    /// </summary>
    [Fact]
    public void MostlyVisibleContent_IsNotClipped()
    {
        // 內容 260×260、畫布 200×200：超出畫布，但看得到的佔大半 → 不裁，快取蓋得到整個物件
        var (session, layer) = NewDoc(new SKRectI(-30, -30, 230, 230));
        AddOutline(session, layer, 4);
        LayerEffectRenderer.RenderLayerNow(session.Document, layer);

        Assert.False(layer.FxCache.LastClipped);
        session.Dispose();
    }
