...
public sealed class Compositor : IDisposable
{
    ...
    private uint[] ReadGroupSourceLocked(GroupLayer group, SKRectI docRect) =>
        TileCompositing.ReadGroupPixelsLocked(group, docRect, _strokeBuffer,
            _floatingProvider?.Invoke(), _detachedProvider?.Invoke() ?? (null, false));

    /// <summary>同步合成整份文件（匯出／縮圖用），內部自行取得文件鎖。</summary>
    public static SKImage RenderComposite(Document doc) => TileCompositing.RenderComposite(doc);
    ...
    internal static uint[] StaticGroupSourceLocked(GroupLayer group, SKRectI docRect) =>
        TileCompositing.ReadGroupPixelsLocked(group, docRect, null, null, (null, false));
    ...
    private bool CompositeGroup(GroupLayer group, SKSurface surface, SKRectI tileRect) =>
        TileCompositing.CompositeGroup(group, surface, tileRect, _strokeBuffer, _batchFloating, _batchDetached);
